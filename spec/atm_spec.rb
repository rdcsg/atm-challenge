...
describe Atm do
  ...
  it 'allows withdrawal if account has enough balance' do
    ...
    expect(subject.withdraw(45, 1234, pre_set_exp_date, account)).to eq expected_output
  end
  ...
  it 'reject withdrawal if account doesn\'t have enough balance' do
    ...
    expect(subject.withdraw(105, 1234, pre_set_exp_date, account)).to eq expected_output
  end
  ...
  it 'rejects withdrawal if ATM has insufficient funds' do
    ...
    expect(subject.withdraw(100, 1234, pre_set_exp_date, account)).to eq expected_output
  end


  it 'reject withdraw if pin is wrong' do
    expected_output = { status: false, message: 'wrong pin', date: Date.today }
    expect(subject.withdraw(50, 9999, pre_set_exp_date, account)).to eq expected_output
  end


  it 'rejects withdrawal if card has expired' do
    allow(account).to receive(:pre_set_exp_date).and_return('12/15')
    expected_output = { status: false, message: 'denied, card expired.', date: Date.today }
    expect(subject.withdraw(6, 1234, pre_set_exp_date, account)).to eq expected_output
  end
